"""Loads config/models.yml — which AI model each image feature uses, what it
costs, and how to shape that model's input.

Model IDs and prices used to be constants on the feature classes and one-off
`os.getenv` calls, so changing either meant a code change, and two places
(the feature and the help text) could disagree about the price. They now come
from one file an operator can edit.
...
Invalid config raises at startup rather than falling back to something silently
different — a wrong model or a wrong price is worse than a failed deploy.
"""
import os
...
# config/models.yml, resolved from this file so cwd doesn't matter
...
def _require(section: dict, feature: str, key: str):
    if key not in section or section[key] is None:
        raise ModelConfigError(f"設定檔缺少 {feature}.{key}")
    return section[key]


def _parse_feature(feature: str, section) -> ModelConfig:
    if not isinstance(section, dict):
        raise ModelConfigError(f"設定檔的 {feature} 必須是一組設定（縮排的 key: value），實際是 {type(section).__name__}")
    ...
    if not isinstance(cost, int) or isinstance(cost, bool) or cost < 0:
        raise ModelConfigError(f"{feature}.cost 必須是 0 或正整數，實際是 {cost!r}")
    ...
    if not isinstance(input_section, dict):
        raise ModelConfigError(
            f"設定檔缺少 {feature}.input——換模型時必須指定該模型的欄位名稱，"
            f"否則呼叫會失敗。範例見 config/models.yml 的註解。"
        )

    image_field = _require(input_section, f"{feature}.input", "image_field")
    if not isinstance(image_field, str):
        raise ModelConfigError(f"{feature}.input.image_field 必須是欄位名稱字串，實際是 {image_field!r}")
    ...
    if not isinstance(extra_input, dict):
        raise ModelConfigError(f"{feature}.extra_input 必須是一組 key: value，實際是 {type(extra_input).__name__}")
...
def _apply_env_overrides(config: ModelConfig) -> ModelConfig:
    ...
    if cost:
        try:
            ...
            raise ModelConfigError(f"環境變數 {prefix}_COST 必須是整數，實際是 {cost!r}")
        logger.info(f"{config.feature}.cost 被環境變數 {prefix}_COST 覆寫為 {cost}")

    return ModelConfig(**{**config.__dict__, **changes})


def load_model_configs(path: str = None) -> dict:
    """Read and validate the whole config file. Raises ModelConfigError if bad."""
    path = path or os.getenv("MODEL_CONFIG_PATH") or _DEFAULT_PATH
    ...
    try:
        ...
        raise ModelConfigError(
            f"找不到模型設定檔：{path}\n"
            f"請確認 config/models.yml 存在，或用 MODEL_CONFIG_PATH 指定位置。"
        )
    except yaml.YAMLError as e:
        raise ModelConfigError(f"模型設定檔 {path} 格式有誤（YAML 解析失敗）：{e}")

    if not isinstance(raw, dict) or not raw:
        raise ModelConfigError(f"模型設定檔 {path} 是空的或格式不對")

    configs = {
        feature: _apply_env_overrides(_parse_feature(feature, section))
        for feature, section in raw.items()
    }
    logger.info("模型設定已載入：" + "、".join(
        f"{name}={c.model}（{c.cost} 點）" for name, c in configs.items()
    ))
    return configs
...
def get_model_config(feature: str) -> ModelConfig:
    """Config for one feature, loading (and caching) the file on first use."""
    global _cache
    if _cache is None:
        _cache = load_model_configs()

    if feature not in _cache:
        raise ModelConfigError(
            f"模型設定檔沒有 {feature} 這一段。已設定的功能：{sorted(_cache)}"
        )
...
def main():
    ...
    Also the local pre-push check:  python3 -m src.core.model_config
    """
    import sys
