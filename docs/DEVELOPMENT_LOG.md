...
## 2026-08-19 — 點數禮物卡：讓付錢的人和用的人分開

**背景**：這個產品最大的付費障礙不是價格，是**付錢的人正好是最不會付錢的人**。
長輩過不了金流那一關——綠界、信用卡、LIFF 登入，任何一步都會讓他放棄。而子女在
母親節願意花幾百塊讓媽媽開心，眼睛都不眨。所以要賣的不是「更多 AI 效果」，是
一條讓子女付款、長輩使用的路。

**做法**：不記名的點數禮物卡。子女開 `/gift` 買 → 拿到卡號 → 傳給長輩 → 長輩在
bot 輸入「兌換」入帳。新增 `gift_cards` 表、`GiftCardService`、`GiftFeature`，
以及三條網頁路由。

**關鍵取捨**：

- **購買頁刻意不走 LIFF**。買的人多半是子女，可能坐在電腦前、甚至不是 LINE 用戶。
  不記名的卡不需要知道買家是誰，多一道 LINE 登入只會多一個放棄點。少了身分驗證
  也不影響安全：建單不會產生任何有價值的東西，金額與點數都由伺服器端的設定決定，
  卡只在**驗過簽章的綠界回調**裡開立。
- **一張訂單表載兩種生意**。`payment_orders.kind` 決定「結清」是發點數還是開卡，
  在那之前（建單、付款、驗簽、只做一次）兩者完全相同。與其複製一套訂單機制，
  不如讓禮物卡繼承已經證明可靠的那條路徑和它的 `credited_at` 保護。代價是
  `subject_id` 要改成 nullable——禮物單在付款當下真的沒有收點的人。
- **兌換的冪等性照抄入帳的做法**：`code` 唯一鍵 + 兌換時鎖列 + `redeemed_at`，
  都在資料庫裡，不靠記憶體（`-w 2` 下記憶體去重本來就有缺口）。
- **卡號用 Crockford base32**（沒有 I/L/O/U），輸入時大小寫、空白、減號全部吸收，
  打成 I/L/O 也自動當 1/0。打字的人常常是 80 歲、對著孫子傳來的截圖看——這不是
  細節，是這個功能能不能被用起來的分界。
- **不設有效期**。台灣商品禮券不得記載使用期限，點數卡適不適用有灰色地帶，
  保守處理的成本很低。
- **兌換只依賴資料庫，與金流是否開著無關**。關掉金流之後，已經賣出去的卡還是要
  兌換得了，否則等於沒收使用者已經付過的錢。

**順手修掉的一個真 bug**：回調結果原本回傳 ORM 物件，session 一關就 detached；
`/gift/card` 正好在 `with` 區塊外讀它。改成在 session 內取快照（`IssuedCard`）再
跨界，與 `_member_dict` 的既有做法一致。單元測試的 fake session 抓不到這種錯，
是對著本機 DB 跑端對端才浮出來的。

**送給誰：只能交給 LINE 決定**。想做的是「買家挑一位家人，bot 直接推播給他」，
但 LINE **沒有**讀取好友清單的 API，push 也只能發給互動過的 userId——第三方無從指名
一個只知道名字的人，這是刻意的隱私設計。能做到的最接近版本是 `liff.shareTargetPicker`：
買家在 LINE 原生的選擇器裡挑人，LINE 代為送出卡片，而挑中的是誰我們自始至終不會知道。
代價是那則卡片由買家的帳號送出、不是 bot 推播；長輩點一下卡片才進到 bot，此時才是真正
的「收到禮物」通知。用一次點擊換掉「先跟長輩要一組代號」的協調成本，划算。

分享刻意做成獨立的一頁（`/gift/share`），不是付款完成頁的一部分：綠界會把瀏覽器帶離
本站再帶回來，LIFF 的執行環境不保證撐得過那一趟；獨立一頁等於重新啟動一次 LIFF，
也讓買家能在電腦上付完款、改用手機開連結分享。

**一個差點放行的陷阱**：完成頁原本有一顆「用 LINE 傳給家人」，連的是開啟 bot 並帶入
兌換訊息的深連結——但那一頁的讀者是**買家**，他點下去就把禮物兌換到自己帳上了。同一個
連結必須放在「送出去的卡片」裡才成立。已改為導向分享頁，並留下測試釘住兩邊各自該有
什麼（`test_gift_command.py` 最後一段）。

**轉換點在「點數不夠」那句話**。原本只說「您現在有 N 點，這個功能需要 M 點」，
是條死路；現在同一則訊息就給出兩條路（自己儲值／請家人買）。這句話的出現頻率
遠高於任何選單，是禮物卡最重要的入口。
